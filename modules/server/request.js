...
function getCustomerObject(customer) {
  ...
  if (
    ...
    customer.addressBook.addresses.length > 0
  ) {
    for (var i = 0, ii = customer.addressBook.addresses.length; i < ii; i++) {
      result.addressBook.addresses.push(
        getAddressObject(customer.addressBook.addresses[i])
      );
    }
  }
  return result;
...
function setCurrency(request, session) {
  ...
  if (
    ...
    (!currentCountry.alternativeCurrencyCodes ||
      currentCountry.alternativeCurrencyCodes.indexOf(
...
function getGeolocationObject(request) {
  ...
  return {
    countryCode: request.geolocation
      ? request.geolocation.countryCode
      : currentLocale.country,
    latitude: request.geolocation ? request.geolocation.latitude : 90.0,
    longitude: request.geolocation ? request.geolocation.longitude : 0.0,
  };
}
