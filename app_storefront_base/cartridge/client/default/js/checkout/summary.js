...
function updateTotals(totals) {
  ...
  if (totals.orderLevelDiscountTotal.value > 0) {
    ...
    $('.order-discount-total').text(
      '- ' + totals.orderLevelDiscountTotal.formatted
    );
  } else {
...
function updateOrderProductSummaryInformation(order) {
  ...
  order.shipping.forEach(function (shipping) {
    ...
    var email = address.emailAddress;
    ...
    if (
      shipping.productLineItems.items &&
      shipping.productLineItems.items.length > 1
    ) {
      $('h5 > span').text(
        ' - ' +
          shipping.productLineItems.items.length +
          ' ' +
    ...
    var isRequired =
      stateRequiredAttr !== undefined && stateRequiredAttr !== false;
    var stateExists =
      shipping.shippingAddress && shipping.shippingAddress.stateCode
    ...
    if (
      ...
      shippingForm
        ...
        .text(order.resources.addressIncomplete);
    }
